...
export default function LayoutFuturistic({ time, mosqueProfile, prayerTimes, nextPrayer, currentDate, currentHijri, prayerConfig, displaySetting, currentBgImage }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full h-[12vh] border-b border-[#00ffcc]/30 bg-black/50 backdrop-blur-md flex items-center justify-between px-10 shadow-[0_5px_30px_rgba(0,255,204,0.1)]">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 relative flex items-center justify-center">
            ...
            {mosqueProfile.logoUrl ? (
              <img src={mosqueProfile.logoUrl} alt="Logo" className="w-12 h-12 rounded-full object-cover z-10" />
            ) : (
              <span className="text-2xl font-bold z-10">{mosqueProfile.name?.charAt(0) || 'M'}</span>
            )}
          </div>
          <div>
            <h1 className="text-4xl font-black tracking-widest text-[#00ffcc] uppercase drop-shadow-[0_0_10px_#00ffcc]">{mosqueProfile.name}</h1>
            <p className="text-sm text-[#00ffcc]/70 tracking-widest uppercase">{mosqueProfile.address}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-4">
            <span className="text-[#00ffcc]/50 text-xs tracking-widest uppercase">SYS.DATE</span>
            <span className="text-2xl font-bold tracking-widest">{currentDate}</span>
          </div>
          <div className="h-px w-full bg-gradient-to-l from-[#00ffcc] to-transparent"></div>
          <div className="flex items-center gap-4">
            <span className="text-[#00ffcc]/50 text-xs tracking-widest uppercase">HIJRI.CAL</span>
            <span className="text-xl font-bold text-white tracking-widest">{currentHijri}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 relative z-10 flex">
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative">
          ...
          {/* Huge Digital Clock */}
          <div className="relative z-10 flex flex-col items-center justify-center bg-black/40 p-12 rounded-3xl backdrop-blur-md border border-[#00ffcc]/20 shadow-[0_0_50px_rgba(0,255,204,0.1)]">
            <div className="text-xs text-[#00ffcc]/70 tracking-[0.5em] mb-4 uppercase">Current Time Coordinates</div>
            <div className="flex items-center text-[14vw] font-black leading-none drop-shadow-[0_0_20px_rgba(0,255,204,0.8)]">
              ...
              <span className={`mx-4 ${pulse ? 'opacity-100' : 'opacity-10'}`}>:</span>
              <span>{format(time, 'mm')}</span>
            </div>
            <div className="mt-4 border-t border-b border-[#00ffcc]/50 py-2 px-8">
              <span className="text-4xl font-bold tracking-[0.5em] text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]">
                {format(time, 'ss')}
              </span>
            </div>
          </div>
        ...
        <div className="w-[30%] h-full flex flex-col justify-center px-10 gap-4 bg-gradient-to-l from-black/80 to-transparent border-l border-[#00ffcc]/20">
          
          {/* Header indicator */}
          <div className="flex items-center gap-4 mb-4">
            <div className="h-4 w-4 bg-[#00ffcc] animate-pulse shadow-[0_0_10px_#00ffcc]"></div>
            <span className="tracking-[0.3em] text-sm font-bold">PRAYER_SCHEDULE</span>
          ...
          {prayers.map((prayer) => {
            ...
            return (
              <div 
                key={prayer.name} 
                className={`relative flex items-center justify-between p-5 transition-all duration-300 border-l-[4px] ${
                  isNext 
                    ? 'bg-[#00ffcc]/20 border-[#00ffcc] shadow-[inset_20px_0_40px_rgba(0,255,204,0.1),_0_0_20px_rgba(0,255,204,0.3)] transform -translate-x-4 scale-[1.02]' 
                    : 'bg-black/40 border-[#00ffcc]/20 hover:bg-[#00ffcc]/5'
                }`}
              >
                {/* Neon decorative corner */}
                <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-[#00ffcc]/50"></div>
                <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-[#00ffcc]/50"></div>

                <div>
                  <h3 className={`text-2xl font-black tracking-widest ${isNext ? 'text-white drop-shadow-[0_0_8px_#ffffff]' : 'text-[#00ffcc]/80'}`}>
                    {prayer.name}
                  </h3>
                  {prayer.iqamah && (
                    <div className="flex items-center gap-2 mt-1">
                      <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                      <p className={`text-xs tracking-[0.2em] ${isNext ? 'text-red-400 font-bold' : 'text-[#00ffcc]/50'}`}>IQOMAH {prayer.iqamah}</p>
                    </div>
                  )}
                </div>
                <div className={`text-4xl font-black tracking-wider ${isNext ? 'text-[#00ffcc] drop-shadow-[0_0_15px_#00ffcc]' : 'text-white/60'}`}>
                  {prayer.time}
                </div>
              </div>
            );
