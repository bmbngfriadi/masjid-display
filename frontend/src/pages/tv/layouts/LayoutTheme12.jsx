  ...
  return (
    <div className="w-full h-full p-[4vh] text-white flex flex-col font-mono bg-black/60 relative">
      ...
      <div className="flex justify-between items-center mb-[4vh]">
        <div className="bg-white/10 backdrop-blur-md px-[3vw] py-[2vh] rounded-[1vw] border border-white/20 flex items-center gap-[2vw]">
          {mosqueProfile.logoUrl && (
            <img src={mosqueProfile.logoUrl} alt="Logo" className="w-[8vh] h-[8vh] object-contain drop-shadow-lg" />
          )}
          <div>
      ...
      <div className="grid grid-cols-5 gap-[1.5vw] mt-[4vh]">
        {prayers.map((prayer) => {
          ...
          return (
            <div 
              key={prayer.id}
              className={`relative flex flex-col items-center justify-center py-[3vh] rounded-[1vw] transition-all duration-300
                ${isNext ? 'bg-cyan-500 text-black shadow-[0_0_3vh_rgba(34,211,238,0.6)] scale-105 border-[0.5vh] border-white' : 'bg-gray-800/80 border border-gray-700 text-gray-300'}`}
            >
